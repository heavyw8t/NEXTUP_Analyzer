...
- Sources differ (breadth analysis, NEXTUP hypothesis, Slither detector)
- Wording differs
- Severity ratings differ
- Locations overlap but are not identical (same function, different lines; same state variable, different write sites)
...
4. Slither-promoted findings defer to any manual finding (breadth or NEXTUP) covering the same root cause. Slither survives only when alone.
...
- Slither survivors use `[SLITHER-N]`.
