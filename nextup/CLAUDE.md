...
> **LOCAL-ONLY VULN DB (v1.1.0)**: Live Solodit API is disabled by default. The unified-vuln-db MCP only exposes the live tool when started with `ENABLE_LIVE_SOLODIT=1`. By default, every unified-vuln-db query hits the local ChromaDB index via `mcp__unified-vuln-db__search` (plus the other local tools: `get_common_vulnerabilities`, `get_attack_vectors`, `get_root_cause_analysis`, `validate_hypothesis`, `get_similar_findings`). Recon and depth prompts must use local search; `search_solodit_live` is not available unless the operator opted in.

> **RAG POLICY (v1.1.0)**: Agent 1A (RAG meta-buffer) is a normal inline agent. Spawn it alongside 1B/2/3 and wait for it like the others. If the unified-vuln-db MCP is not installed or the local ChromaDB index is empty, the agent's probe call fails, the agent sets `RAG_TOOLS_AVAILABLE=false`, writes an empty `meta_buffer.md`, and returns; Phase 4b.5 RAG Sweep compensates later with the WebSearch fallback. Local ChromaDB queries are fast, so there is no need to run 1A in the background.
